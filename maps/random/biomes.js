	...
	for (let i_t = 0; i_t < teams.length; i_t++)
	{
		for (let i = 0; i < teams[i_t].length - 1; i++)
		{
			for (let j = i + 1; j < teams[i_t].length; j++)
			{
				...
				createPassage({
					"start": playerPosition[playerIDs.indexOf(teams[i_t][i])],
					"end": playerPosition[playerIDs.indexOf(teams[i_t][j])],
					"startWidth": rWidth,
					"endWidth": rWidth,
					"smoothWidth": 0,
					"tileClass": clRoad,
				...
				let area = createArea(
					new PathPlacer(
						playerPosition[playerIDs.indexOf(teams[i_t][i])],
						playerPosition[playerIDs.indexOf(teams[i_t][j])],
						rWidth, // width
