...
DunePainter.prototype.paint = function(area)
{
	...
	for (var i = 0; i < length; i++)
	{
		...
		const h1 = this.dune(v1, 0.6, false) * 9 * 0.9 + h1_1[i] * h1_2[i] * 150 * 0.1;
		// secondary dunes
		const v2 = ((points[i].x + h2_1[i] * 13 + points[i].y * 0.1) % L2) / L2;
		const h2 = this.dune(v2, 0.6, false) * 9 * 0.9 + h2_2[i] * 50 * 0.1;
