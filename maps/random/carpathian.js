...
function AreaMap(mapTypePar, fieldSizePar) {
	// index 0..19 are reserved for global custom typology
	// index 20..49 are reserved for local custom typology
	var index = 50;
	...
	// for (var j=0; j < dbrushB.length; j++) {
		...
		if (0 <= xmm && xmm < dd && 0 <= ymm && ymm < dd) {
			if ((xmm + ymm) % roC == ro) {
				...
				// everything that is smaller is potential slope

				// var wdiff = Math.abs(self.d[0].w - self.d[1].w);
				// if (wdiff > Math.PI) wdiff = Math.abs(2 * Math.PI - wdiff);
				// var relSize = Math.cos(0.5 * wdiff) * self.d[0].d;
				// anything relSize < 2 is potential path
			...
			for (var ro = 0; ro < roC; ro += 1) {
				for (var j=0; j < dbrushB.length; j++) {
					...
					for (var i = 0; i < hills.length; i += 1) {
						...
						// making the hills is one of the most time consuming tasks, that's why there are regularly updates
