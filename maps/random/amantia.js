...
// value should be more than 5, otherwise there are to few ramps to access high or low terrain.
var vorWidth =5;
var cliffHeight = 22;
// ramp base
var rampSize = 3;
...
var areaMapTyp = 45;
// maximum distance (to the square) of two hills to allow a ramp between both
// a value of 4000 implies, that the two hills have to be at least 20 fields away,
// thus create only ramps if there is enough distance or if they are explicitely requested
// at the end of the log the smallest two maximum Distances between between two hills is displayed
// the choosen value should be smaller than the first value
// if this value is to big, there will be the risk of small cells having no ramp at all.
var preventRampDistance = 2500;
 ...
 * cells of the type hill are active cells with 
	...
	#	marker
	!	active basefield, level 0
	?	active basefield, no slopes, level 0
	a	active basefield, level 1
	A	active basefield, no slopes, level 1
	b	active basefield, level 0, no slopes from Nonbasefields
	B	active basefield, level 1, no slopes from Nonbasefields
	...
	free:
		cCdDeEhHiIjJkKlLmMnNpPqQrRsStTuUvVzZ
*/
...
mapLayout[0] = tinyMap0;
mapLayout[1] = tinyMap1;
mapLayout[2] = test1;
mapLayout[3] = test2;
mapLayout[4] = test5;
mapLayout[5] = test5;
mapLayout[6] = tinyMap2;
mapLayout[7] = emptyMap0;


if (true) {
	mapAssign[128] = [0,0,1,1,0,0,0,0,0];
	mapAssign[192] = [0,0,6,6,1,1,1,0,0];
	mapAssign[256] = [0,0,2,2,3,1,1,1,1];
	mapAssign[320] = [0,0,5,5,5,5,3,3,3];
	mapAssign[384] = [0,0,5,5,5,5,5,3,2];
	mapAssign[448] = [0,0,5,5,5,5,5,5,5];
	mapAssign[512] = [0,0,5,5,5,5,5,5,5];
}
...
var texture_trees1_inner_high = ["grid_subdiv"];
...
var treasure_stone = "gaia/treasure/stone_pile_granite_large";
var treasure_metal = "gaia/treasure/metal_persian_big";

var struct_house_1 = "structures/brit/house";
var struct_house_2 = "structures/stall_big";
var struct_house_3 = "structures/stall_med";
...
if (true) {
	...
	texture_default = ["temperate_grass_01", "temperate_grass_01", "temperate_grass_02"];
	texture_default_medium = ["temperate_grass_01", "temperate_grass_02", "temperate_grass_02"];
	// depression, elevation of forests above the ground

	texture_depression_inner = ["temperate_mud_01"];
	texture_depression_outer = ["temperate_grass_mud_01"];
	height_depression = -2;
	...
	texture_trees1_inner = ["temperate_forestfloor_01", "temperate_forestfloor_02", "temperate_grass_dirt_01"];
	texture_trees1_inner_high = ["temperate_forestfloor_01", "temperate_forestfloor_02", "temperate_grass_dirt_01"];
	texture_trees1_outer = ["temperate_forestfloor_01", "temperate_grass_dirt_01", "temperate_grass_dirt_02", "temperate_grass_dirt_01", "temperate_grass_dirt_02"];
	texture_trees1_outer_high = ["temperate_forestfloor_02", "temperate_grass_dirt_01", "temperate_grass_dirt_02", "temperate_grass_dirt_01", "temperate_grass_dirt_02"];
	objects_trees1_inner = ["gaia/tree/oak_large", "gaia/tree/oak_new", "gaia/tree/bush_temperate", "gaia/tree/oak_large", "gaia/tree/oak_new", "gaia/tree/bush_temperate", "gaia/tree/oak", "gaia/tree/oak_holly"];
	objects_trees1_inner_high = ["gaia/tree/oak", "gaia/tree/oak_holly", "gaia/tree/bush_temperate", "gaia/tree/oak", "gaia/tree/oak_holly", "gaia/tree/bush_temperate", "gaia/tree/oak_large", "gaia/tree/oak_new"];
	height_trees1 = 2;
	margin_trees1 = 20;
	// size_trees1 = diameter of a forest
	...
	// cliffs
	texture_cliff1_inner = ["temperate_cliff_01", "temperate_cliff_01", "temperate_cliff_02"];
	texture_cliff1_inner_high = ["temperate_cliff_01", "temperate_cliff_02_dover"];
	texture_cliff1_outer = ["temperate_rocks_dirt_01"];
	texture_cliff1_outer_high = ["temperate_rocks_dirt_01"];

	// path
	texture_path = ["temp_road", ""];
	texture_path_high = ["temp_road_aut", ""];

	// texture around the base and the mines
	texture_base_inner = ["temp_dirt_gravel", "temp_dirt_gravel_b", "temperate_grass_dirt_03", "temperate_grass_dirt_04", "temp_farmland"];
	texture_base_outer = ["temp_dirt_gravel_plants", "temp_dirt_gravel_plants", "temp_farmland", "temp_farmland", "temperate_grass_dirt_03", "temperate_grass_dirt_04"];
	texture_mine = ["temperate_rocks_dirt_01"];
	// special objects
...
function renderCell(self, pass) {
	...
	if (pass == 0) {
		par1 = 3 * Math.cos((0.2 + 0.04 * Math.sin(5*w)) * d) + 3 * Math.cos((0.15 + 0.035 * Math.sin(11*w)) * d);
	...
	if (pass == 0 && typeof self.d != "undefined") {
		...
		if (self.d.length > 0) {
			...
			if (testColoring == 7) {
				if (self.d[0].p % 1000 < 50) {
					textureOverwrite = color_2;
					if (!slopeMake) textureOverwrite = color_4;
				}
			}
		}
		if (self.d.length > 1) {
			// if a field have the values of two cells, than there is maybe a ramp
			...
			if (elevationLevel != elevationLevelSec) {
				if (_ifp0 < 50 && _ifp1 < 50  && _ifplayer0 == _ifplayer1) {
					...
					if (hill1 != null && hill2 != null) {
						if (!hill1.slope || !hill2.slope) {
							// in a base from different levels to an antislop field should not be accessable from other interim-levels
							// @TODO does not work
							// vorWidthIntern=1;
						}
					}
			...
			if (hill1 != null && hill2 != null) {
				// g_Map.log("hillsDefined");
				if (_ifp0 >= 50 && _ifp1 < 50) {
					// base blocks ramps from non-base hills
					// g_Map.log("hills oppose base " + JSON.stringify(hill1) + " " + JSON.stringify(hill2) );
					if (hill2.antiSlope) slopeMake = false;
				}
				if (_ifp0 >= 50 && _ifp1 >= 50) {
					// calculate the distance between the two hills
					// for speed the calculation of the root is ommitted
					// don't calculate the distance if one of the hills is a base,
					// 	because bases tend to have their own handling of ramps
					var ddHill = (hill1.x - hill2.x)*(hill1.x - hill2.x) + (hill1.y - hill2.y)*(hill1.y - hill2.y);
					// g_Map.log("hills Distance: " + ddHill + ' ' + hill1.owner + ' ' + hill2.owner);
					if (!(hill1.owner in hillsDistance)) hillsDistance[hill1.owner] = {};
					if (!(hill2.owner in hillsDistance)) hillsDistance[hill2.owner] = {};
					if (!(hill2.owner in hillsDistance[hill1.owner])) hillsDistance[hill1.owner][hill2.owner] = ddHill;
					if (!(hill1.owner in hillsDistance[hill2.owner])) hillsDistance[hill2.owner][hill1.owner] = ddHill;
					if (ddHill < preventRampDistance) slopeMake = false;
				}
			}
			else {
				g_Map.log("hillsDefined not");
			}
			...
			if (makeRim) {
				...
				var vorWidthV = vorWidthIntern + par2;
				if (diff < vorWidthV) {
					// interim level
					h = cliffHeight + par1;
				}
				if (slopeMake) {
					...
					var slopeStart = 1.2 + 0.25 * par2Raw;
					// var slopeAscent = 0.25;
					// mathematical not totally correct
					// var rampBroadFactor = Math.max(1, (rampSize - relSize - 2) / rampSize);
					// var rampFactor = Math.max(0.0, Math.min(1.0, (slopeStart * vorWidth - slopeAscent * diff) / vorWidth));
					var rampFactor = Math.max(0.0, Math.min(1.0, 1.0 - 0.035 * (diff - 0.8 * vorWidthV)));
					h = (cliffHeight + 0.25 * par1) * rampFactor + hPlateau * (1 - rampFactor);
					
					if (rampSize > relSize && rampFactor > 0.05) {
	...
	if (pass == 1 && testColoring == 0) {
		...
		if (0.4 < self.base) {
			...
			if (0.5 * Math.random() < self.base - 0.3) {
				textureT = pickRandom(texture_base_outer);
			}
			if (0.1 * Math.random() < self.base - 0.7) {
				textureT = pickRandom(texture_base_inner);
			}
	...
	if (0.0 < self.forest2 && testColoring == 0) {
		...
		if (pass == 1) {
			...
			if (5.0 * Math.random() < self.forest2 + density && self.path < 0.5 && self.base < 0.6) {
				textureT = pickRandom(texture_trees1_inner);
				element = pickRandom(objects_trees1_inner);
				if (timberline > 0) {
					element = pickRandom(objects_trees1_inner_high);
					textureT = pickRandom(texture_trees1_inner_high);
				}
			}
			if (1 < textureT.length) texture = textureT;
		}
...
function AreaMap(mapTypePar, fieldSizePar) {
		...
		for (var maxwi = -maxw; maxwi < maxw + 1; maxwi += 1) {
			...
			if (legal && d <= mapSizeDist2) {
				...
				if (0.275 * fieldSize < wTest) {
					...
					//	slope	...
					field[maxhi][maxwi] = {'id': index, 'x': x, 'y': y, 'rx': x, 'ry': y, 'type': '', 'active': false, 'slope': true, 'antiSlope': false, 'ra': defaultRandom, 'height': 1};
					index += 1;
	...
	this.setMap = function(mapData) {
		// set the mapdata/layout given by the designer,
		// complement missing fields by the defaults or reduce mapdata, if they are to long
		...
		for (var hi = 0; hi < him.length; hi += 1) {
			...
			for (var wi = 0; wi < wim.length; wi += 1) {
				...
				if (f.active) {
					...
					result.push({'x': t.x, 'y': t.y, 'owner': globalIndex, 'elevation': f.height, 'slope': f.slope, 'antiSlope': f.antiSlope});
				}
			}
		}
		return result;		
			...
			if (1 < areaBoundaries.length) {
				for (var ix = areaBoundaries[0]; ix < areaBoundaries[1] + 1; ix += 1) {
					...
					if (fa != null) {
						...
						fa.ra = 2.2;
						if (fa.type.length > 0 ) {
							if ('!?aAbB'.indexOf(fa.type) > -1) {
								_idBase +=1
							}
							if ('aAbBfguwxyFGW.!?#'.indexOf(fa.type) > -1) {
								fa.ra = 0;
							}
						...
						if (fa.type.length > 0 ) {
							...
							if ('!?aAbB'.indexOf(fa.type) > -1) {
								...
								if ('aAB'.indexOf(fa.type) > -1) {
									field[iy][ix].height = 1;
								}
								if ('?A'.indexOf(fa.type) > -1) {
									field[iy][ix].slope = false;
								}
								if ('bB'.indexOf(fa.type) > -1) {
									field[iy][ix].antiSlope = true;
								}
								isMappedElement = 2;
							}
							...
							if ('U'.indexOf(fa.type) > -1) {
								var ms1 = self.setElementGaia(ix,iy,treasure_stone,-2);
								var ms1 = self.setElementGaia(ix,iy,treasure_metal,+2);
								field[iy][ix].id = _id;
								field[iy][ix].active = false;
								isMappedElement = 2;
							}
							if ('#'.indexOf(fa.type) > -1) {
								// marker
								var ms1 = self.setElementGaia(ix,iy,marker1,0,0);
								field[iy][ix].id = _id;
								field[iy][ix].active = false;
							}
		...
		if (0 <= xmm && xmm < dd && 0 <= ymm && ymm < dd) {
			if ((xmm + ymm) % roC == ro) {
				...
				if (map[ymm][xmm].d.length == 0) {
					...
					// here actually is the width of the ramp implemented
...
if (true) {
	...
	if (true) {
		...
		// set the layout and adapt it to the field
		a.setMap(mapLayout[mapAssign[dd][numPlayers]]);
		
		for (var i = 1; i < (numPlayers + 1); i++) {
			var aa = a.setPlayer(i);
			// calculate all the hills and all the other stuff
			// explicitly set objects are added to the map
			// the data for the hills is set
			// but the hills are turned into topographie further down
			aa.placeObjects();
		...
		for (var jj=0; jj < saveDistanceLoop; jj += 1) {
			// displace hills to ensure a save distance
			var moveDistance = 0;
			var replace = {}
	...
	g_Map.log("generating textures and random objects");
	for (var i=0; i<dd; i++) {
		for (var j=0; j<dd; j++) {
			if (typeof map[i][j] != "undefined") {
				// g_Map.log("rendering:" + i + " " + j);
...
// loop through the distance of all Hills
// look for the smallest distance of two hills, measured by their largest distance to each other
var hillsDistanceKeys = Object.keys(hillsDistance);
var maxHillDist0 = 10000, maxHillDist1 = 100000;
for (var i = 0; i < hillsDistanceKeys.length; i += 1) {
	var hillsDistanceKey = hillsDistanceKeys[i];
	var valuesHillDD = Object.values(hillsDistance[hillsDistanceKey]);
	valuesHillDD.sort(function(a,b){return a - b});
	var ddLen = valuesHillDD.length;
	// g_Map.log("hillsDistance:" + hillsDistanceKey + " " + JSON.stringify(hillsDistance[hillsDistanceKey]) + " " + JSON.stringify(valuesHillDD));
	if (0 < ddLen && valuesHillDD[ddLen - 1] < maxHillDist0) maxHillDist0 = valuesHillDD[ddLen - 1];
	if (1 < ddLen && valuesHillDD[ddLen - 2] < maxHillDist1) maxHillDist1 = valuesHillDD[ddLen - 2];
}
g_Map.log("maxDistBetweenHills:" + maxHillDist1 + " " + maxHillDist0);
g_Map.ExportMap();
