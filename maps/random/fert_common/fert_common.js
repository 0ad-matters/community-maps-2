...
const genMapFertWithMountain = function(isMountain)
{
	setSelectedBiome();
	...
	{
		const v0 = p0.clone().mult((1 - t) * (1 - t));
		const v1 = p1.clone().mult(2 * (1 - t) * t);
		const v2 = p2.clone().mult(t * t);
	...
	{
		const startAngle = 2 * Math.PI * t / getNumPlayers();
		...
		if (isMountain)
		{
			const endAngle = startAngle + mountain.twist;
			const shaper = a => clamp(mountain.heightBase + mountain.height * a * 1.1, 0, mountain.heightBase + mountain.height);
			const setcondition = (p, h) => g_Map.getHeight(p) < h;
		...
		const anglemaxh = anglemin + littlePassageRadPod;
		const anglemax = anglemin + arcAngleLength;

		// Circular strip from spiral to border
		const shaper0 = (a, r) => mountain.heightBase * (1 - Math.pow(easeCurve(a), 5));
		const setCondition0 = (p, h) => g_Map.getHeight(p) < h;
		...
		const shaper1 = (a, r) => shaper0(a, r) * easeCurve(1 - r);
		const setCondition1 = (p, h) => g_Map.getHeight(p) < h;
		...
		const shaper2 = (a, r) => -40 * easeCurve(a) * Math.pow(r, 1.3);
		const setCondition2 = (p, h) => g_Map.getHeight(p) < 0.1;
		...
		const shaper3 = (a, r) => -4 + easeCurve(Math.abs(r * 2 - 1)) * 2;
	...
	{
		const startAngle = 2 * Math.PI / getNumPlayers() * scaleByMapSize(0.1, 0.3);
		const [playerPosition, playerAngle] = distributePointsOnCircle(getNumPlayers(), startAngle, radius * scaleByMapSize(0.9, 1.4), center || g_Map.getCenter());
	...
	{
		const startRoad = spiralEndPos[i];
		// civsPos indexes doesn't match with spiralEndPos ones.
		const endRoad = civsPos[(getNumPlayers() - i + 1) % getNumPlayers()];
		...
		{
			const unitary = step / nSubpaths;
			const unitaryNext = (step + 1) / nSubpaths;
			const point = bezier_quadratic(startRoad, controlPoint, endRoad, unitary);
			const pointNext = bezier_quadratic(startRoad, controlPoint, endRoad, unitaryNext);
